...
DEFAULT_ROLE_PATTERNS: dict[str, tuple[str, ...]] = {
    'aodv': ('src/main.py node --config', 'main.py node --config'),
...
ROLE_ORDER = ('aodv', 'video_forwarder', 'overlay_bench')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--role',
        choices=('all',) + ROLE_ORDER,
        default='all',
        help='Limit output to one process role. Default: all.',
    )
    parser.add_argument(
        '--json',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help='Refresh interval in seconds. Use with --samples for repeated snapshots.',
    )
    parser.add_argument(
        '--samples',
        type=int,
        default=1,
        help='Number of snapshots to capture when --watch-sec > 0. Default: 1.',
    )

...
class VideoForwarder:
    ...
    def send_packet(self, packet: dict[str, Any]) -> None:
        ...
        self.log(f'send kind={packet.get("kind")} transfer_id={packet.get("transfer_id")} next_hop={next_hop_ip} final_dest={dest_ip}')

    def wait_for_route(self, dest_ip: str) -> str:
        deadline = time.time() + self.route_timeout_sec
        first_attempt = True
    ...
    def wait_for_ack(self, transfer_id: str, ack_for: str, timeout_sec: float, chunk_id: int = -1) -> dict[str, Any]:
        ack_queue = self.register_ack_waiter(transfer_id, ack_for, chunk_id)
        try:
            return ack_queue.get(timeout=timeout_sec)
        finally:
            self.pop_ack_waiter(transfer_id, ack_for, chunk_id)
    ...
    def handle_packet(self, raw: bytes, remote_addr: tuple[str, int]) -> None:
        try:
    ...
    def run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            for sock in readable:
                raw, remote_addr = sock.recvfrom(65535)
                self.handle_packet(raw, remote_addr)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Independent UDP video/file forwarder using AODV only for route lookup.')
    parser.add_argument('--node-ip', required=True, help='Local node IP known by the AODV overlay.')
